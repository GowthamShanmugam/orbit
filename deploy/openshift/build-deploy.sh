...
#
# Auth (single sign-in): Red Hat SSO / Keycloak is the identity provider. oauth-proxy (if present)
# only gates the app and forwards X-Forwarded-* headers; it is not a second login.
# Logout: default Orbit image does not redirect to /oauth2/sign_out (nginx would serve the SPA).
# Set OCP_OAUTH_SIGNOUT_URL to your IdP logout URL, or OCP_OAUTH_SIGNOUT_PATH only if oauth-proxy
# handles that path before the frontend Service.
set -euo pipefail
...
echo ""
echo "If the API returns 500s (e.g. column projects.runtime_overrides does not exist), apply DB migrations:"
echo "  $ROOT/deploy/openshift/alembic-upgrade.sh"
echo ""
