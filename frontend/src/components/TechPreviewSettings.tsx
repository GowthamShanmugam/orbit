...
export default function TechPreviewSettings({ project }: Props) {
  const queryClient = useQueryClient();
  const flags: Record<string, boolean> = project.feature_flags ?? {};

  const toggleMut = useMutation({
    mutationFn: (patch: Record<string, boolean>) =>
      updateProject(project.id, { feature_flags: patch }),
    onSuccess: (updated) => {
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-xs text-[var(--o-text-tertiary)]">
          Enable experimental features for this project. These may change or be removed.
        </p>
      </div>
