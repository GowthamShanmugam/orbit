...
interface ChatPanelProps {
  projectId: string;
  sessionId: string;
  /** Project's organization — enables shared team prompts. */
  orgId?: string | null;
...
export default function ChatPanel({
      ...
      if (existing) {
        ...
        try {
          ...
          if (msg.includes("409")) {
            ...
            const found = threads.find((t) => t.parent_message_id === message.id);
            if (found) {
  ...
  const handleStream = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        queryClient.invalidateQueries({
          queryKey: ["artifacts", projectId, sessionId],
        });
      }
    },
    [
  ...
  const selectedLabel = MODELS.find((m) => m.id === model)?.label ?? MODELS[0].label;

  return (
    <div className="relative flex h-full min-h-0 w-full flex-col bg-[var(--o-bg-overlay)]">
      {activeThread && <ThreadPanel projectId={projectId} sessionId={sessionId} />}
      ...
      <div ref={listRef} className="min-h-0 flex-1 space-y-3 overflow-y-auto px-3 py-3">
        {messages.length === 0 && !isStreaming && (
          <div className="flex h-full flex-col items-center justify-center gap-3 py-12 text-center">
            <div
              className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[var(--o-accent-muted)]"
                  ...
                  <div className="mt-1 flex items-center gap-1.5">
                    {threadForMsg ? (
                      ...
                      >
                        ...
                        {threadForMsg.reply_count}{" "}
                        {threadForMsg.reply_count === 1 ? "reply" : "replies"}
                      </button>
                    ) : (
                      <button
                        type="button"
      ...
      <div className="shrink-0 border-t border-[var(--o-border)] bg-[var(--o-bg-raised)] p-3">
        {readOnly ? (
          ...
          <>
            <div className="mb-2">
              <SkillSelector projectId={projectId} sessionId={sessionId} />
            </div>
            {orgId && (promptsQuery.data?.templates.length || promptsQuery.data?.can_manage) && (
              <div className="mb-2 flex flex-wrap items-center gap-1.5">
                <span className="text-[10px] font-semibold uppercase tracking-wide text-[var(--o-text-tertiary)]">
                  Team prompts
                </span>
                {promptsQuery.isLoading && (
                  <span className="text-[11px] text-[var(--o-text-tertiary)]">…</span>
                )}
                {(promptsQuery.data?.templates ?? []).map((t) => (
                  <button
                    key={t.id}
                    type="button"
                    title={t.title}
                    onClick={() => setDraft(t.body)}
                    className="max-w-[140px] truncate rounded-full border border-[var(--o-border)] bg-[var(--o-bg-subtle)] px-2.5 py-1 text-left text-[11px] font-medium text-[var(--o-text)] transition-colors hover:border-[var(--o-accent)]/50 hover:bg-[var(--o-accent-muted)] hover:text-[var(--o-accent)]"
                  >
                    {t.title}
                  </button>
                ))}
                {promptsQuery.data?.can_manage && (
                  <button
                    type="button"
                    onClick={() => setPromptsModalOpen(true)}
                    className="inline-flex items-center gap-1 rounded-full border border-dashed border-[var(--o-border)] px-2 py-1 text-[11px] text-[var(--o-accent)] transition-colors hover:bg-[var(--o-accent-muted)]"
                  >
