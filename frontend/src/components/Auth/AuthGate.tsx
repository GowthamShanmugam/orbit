...
export default function AuthGate({ children }: { children: React.ReactNode }) {
  const { isAuthenticated, login, logout, setAuthContext } = useAuthStore();
  ...
  useEffect(() => {
    async function bootstrap() {
      try {
        const { mode, ocpSignoutPath, ocpSignoutUrl } = await getAuthMode();
        setAuthMode(mode);
        setAuthContext({ authMode: mode, ocpSignoutPath, ocpSignoutUrl });
      ...
      <div className="w-full max-w-sm" style={{ boxShadow: "var(--o-shadow-xl)" }}>
        <div className="rounded-2xl border border-[var(--o-border)] bg-[var(--o-bg-raised)] p-8 text-center">
          ...
          <p className="mt-2 text-sm text-[var(--o-text-secondary)]">
            Sign in once with your cluster SSO (oauth-proxy only forwards you there — it is not a
            second login). Refresh to continue.
          </p>
          <button
            type="button"
