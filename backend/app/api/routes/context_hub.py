...
class PackResponse(BaseModel):
    ...
    """How many projects have this pack installed (detail fetches set this)."""


class PackInstallationRow(BaseModel):
    project_id: UUID
    project_name: str
...
async def get_pack(
    ...
    n = await hub_svc.count_pack_installations(db, pack_id)
    base = PackResponse.model_validate(pack)
    return base.model_copy(update={"installation_count": n})


@router.get(
    "/packs/{pack_id}/installations",
    response_model=list[PackInstallationRow],
)
async def list_where_pack_installed(
    pack_id: UUID,
    current: Annotated[User, Depends(get_current_user)],
    db: Annotated[AsyncSession, Depends(get_db)],
) -> list[PackInstallationRow]:
    pack = await hub_svc.get_pack(db, pack_id)
    if pack is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Pack not found")
