...
_ORBIT_PACK_ID_KEY = "_orbit_pack_id"
...
async def list_pack_installation_projects(
    db: AsyncSession, pack_id: uuid.UUID
) -> list[tuple[uuid.UUID, str]]:
    from app.models.project import Project

    stmt = (
        select(InstalledPack.project_id, Project.name)
        .join(Project, Project.id == InstalledPack.project_id)
        .where(InstalledPack.pack_id == pack_id)
...
async def list_packs(
    ...
    *,
    ...
    if limit is None:
        limit = settings.CONTEXT_HUB_PACK_LIST_DEFAULT
    stmt = (
        select(ContextPack)
...
async def delete_pack(db: AsyncSession, pack: ContextPack) -> None:
    if await count_pack_installations(db, pack.id) > 0:
        raise ValueError(
            "Pack is installed on one or more projects; uninstall it from every project first.",
        )
...
async def install_pack(
    ...
    *,
    ...
    for src in pack.sources:
        merged_cfg: dict[str, Any] = dict(src.config) if src.config else {}
        merged_cfg[_ORBIT_PACK_ID_KEY] = pid_str
        merged_cfg["_orbit_pack_source_id"] = str(src.id)
        ctx = ContextSource(
            project_id=project_id,
            type=src.type,
            name=src.name,
...
async def uninstall_pack(
    ...
    for row in rows:
        cfg = row.config or {}
        if cfg.get(_ORBIT_PACK_ID_KEY) == pid_str:
            to_delete.append(row)
            continue
        if cfg.get(_ORBIT_PACK_ID_KEY):
            continue
        if pack and (row.type, row.name, row.url or None) in triples:
            to_delete.append(row)
