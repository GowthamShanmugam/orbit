...
async def _get_resources(
    ...
    data = await kube_client.get_resources(
        ...
        limit=settings.KUBE_TOOL_DEFAULT_LIMIT,
    )
...
async def _get_logs(
    ...
    max_c = settings.KUBE_LOG_MAX_CHARS
    if len(logs) > max_c:
        logs = logs[-max_c:]
        logs = "...(truncated)\n" + logs
    return logs
...
async def _run_command(
    ...
    job_manifest: dict[str, Any] = {
        ...
        "spec": {
            "backoffLimit": 0,
            "activeDeadlineSeconds": settings.KUBE_JOB_ACTIVE_DEADLINE_SECONDS,
            "ttlSecondsAfterFinished": settings.KUBE_JOB_TTL_SECONDS_AFTER_FINISHED,
            "template": {
                "metadata": {
                    "labels": {"app.kubernetes.io/managed-by": "orbit"},
                },
...
def _summarise_resources(items: list[dict[str, Any]], resource_type: str) -> list[dict[str, Any]]:
    ...
    for item in items:
        ...
        if resource_type == "pods":
            ...
            summary["message"] = item.get("message", "")[
                : settings.KUBE_STATUS_MESSAGE_PREVIEW_CHARS
            ]
...
async def _wait_for_job_logs(
    ...
    if timeout is None:
        timeout = settings.KUBE_WAIT_POD_TIMEOUT_SEC
    ...
    logs = await kube_client.get_logs(
        ...
        pod_name,
        namespace=namespace,
        tail_lines=settings.KUBE_DIAG_LOG_TAIL_LINES,
    )
    max_c = settings.KUBE_LOG_MAX_CHARS
    if len(logs) > max_c:
        logs = logs[-max_c:]
