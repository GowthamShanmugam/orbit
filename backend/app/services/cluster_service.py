...
# Sentinel: parameter was not passed (vs explicit ``None`` for nullable columns).
_MISSING = object()
...
async def update_cluster(
    ...
    *,
    name: Any = _MISSING,
    namespace_filter: Any = _MISSING,
    sync_config: Any = _MISSING,
    credentials: Any = _MISSING,
    api_server_url: Any = _MISSING,
) -> ProjectCluster:
    if name is not _MISSING:
        if not isinstance(name, str) or not name.strip():
            raise ValueError("name must be a non-empty string")
        cluster.name = name.strip()
    if namespace_filter is not _MISSING:
        cluster.namespace_filter = namespace_filter
    if sync_config is not _MISSING:
        cluster.sync_config = sync_config
    if api_server_url is not _MISSING:
        cluster.api_server_url = api_server_url
    if credentials is not _MISSING and credentials is not None:
        ct, nonce, tag = encrypt_credentials(credentials)
...
async def test_connection(cluster: ProjectCluster) -> tuple[bool, str]:
    ...
    try:
        ...
        async with httpx.AsyncClient(
            verify=verify_ssl, timeout=settings.CLUSTER_TEST_TIMEOUT_SEC
        ) as client:
...
async def list_test_runs(
    ...
    if limit is None:
        limit = settings.CLUSTER_TEST_RUN_LIST_LIMIT
    stmt = (
        select(TestRun)
